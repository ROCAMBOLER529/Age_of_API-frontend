...
const LogInLink = datos => {
    ...
    const llamadaAlPost = async () => {
        try {
            const res = await fetch(`https://age-of-api-backend.onrender.com/api/auth/logIn/`, {
                method: 'POST',
                headers: {
    ...
    return (
        <TryItLink className="button" onClick={llamadaAlPost} to="/" activeStyle>LogIn</TryItLink>
    );
}
